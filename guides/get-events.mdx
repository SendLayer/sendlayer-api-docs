...
events, err := sl.Events.Get(&start, &end, "", "", nil, nil)
```
...
opened, err := sl.Events.Get(nil, nil, "opened", "", nil, nil)
bounced, err := sl.Events.Get(nil, nil, "bounced", "", nil, nil)
```
...
events, err := sl.Events.Get(nil, nil, "", msgID, nil, nil)
```
...
startFrom := 10
retrieveCount := 20
events, err := sl.Events.Get(nil, nil, "", "", &startFrom, &retrieveCount)
```
...
events, err := sl.Events.Get(&start, &end, "opened", "", nil, &retrieveCount)
```

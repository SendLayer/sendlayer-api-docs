...
### Sending an Email
Create a new `.py` file or edit an existing one. Then import the `NewEmail` module. Then initialize the module with your SendLayer API key.
...
You can include attachments to your email message.
...
Simply replace the path parameter with the path to the attachment file you wish to attach. SendLayer SDK supports both local and remote file attachments.
...
| `from_email` | `string` | Yes | Email address of the sender |
| `from_name` | `string` | No | Name of the sender |
...
To create a new webhook, you'll need to first import the Webhooks module and initialize it with your API key.

Then call the `create()` method and specify the required parameters. This method requires the `url` and `event` parameters.
...
To view all the webhooks you've created, use the `get_all()` method.
...
#### Deleting a Webhook
To delete a specific webhook, use the `delete()` method. This method accepts one required parameter `webhook_id`. Which needs to be a number.
...
#### Getting All Events
To get started, import the `Events` module and initialize it with your API key. Then call the `get()` method to retrieve all events.

<Note> The `get()` method retrieves the top 5 events in your account if no filter parameter is specified.</Note>
...
#### Filtering Events
The `get()` method in the `Events` module accepts some optional parameters. These parameters can be used to filter the API response. Here is an example:
